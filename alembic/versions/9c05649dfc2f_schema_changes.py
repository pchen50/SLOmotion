...
def upgrade() -> None:
    ...
    op.add_column("users", sa.Column("username", sa.String(100), nullable=False))
    op.create_unique_constraint("username_uq", "users", ["username"])
    op.create_foreign_key(
        "commenter_foreign", "comments", "users", ["commenter_user_id"], ["id"]
    )
    op.create_unique_constraint(
        "comments_uq", "comments", ["commenter_user_id", "user_id", "movie_id"]
    )
    ...
    op.create_check_constraint(
        "rating_between_0_10", "movie_ratings", "rating >= 0 AND rating <= 10"
    )
    op.create_unique_constraint(
        "movie_rating_uq", "movie_ratings", ["movie_id", "user_id"]
    )
